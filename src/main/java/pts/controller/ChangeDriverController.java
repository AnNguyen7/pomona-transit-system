...
public class ChangeDriverController {
    ...
    @FXML private TextField txtNewDriver;
    ...
    private void handleUpdate() {
        ...
        String newDriver = txtNewDriver.getText().trim();

        if (tripNum.isEmpty() || datePicker.getValue() == null || startTime.isEmpty() || newDriver.isEmpty()) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rowsAffected > 0) {
                showAlert("Success", "Driver changed successfully!", Alert.AlertType.INFORMATION);
                txtTripNumber.clear();
